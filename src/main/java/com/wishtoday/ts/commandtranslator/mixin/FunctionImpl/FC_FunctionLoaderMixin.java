...
public abstract class FC_FunctionLoaderMixin {
    ...
    private CompletableFuture<CommandFunction<ServerCommandSource>> wrap(Supplier<CommandFunction<ServerCommandSource>> supplier, Executor executor, Operation<CompletableFuture<CommandFunction<ServerCommandSource>>> original, @Local Map.Entry<Identifier, Resource> entry, @Local ServerCommandSource source, @Local(ordinal = 1) Identifier identifier) {
        ...
        Optional<FunctionTranslationProvider> provider = Container.getInstance().get(FunctionTranslationProvider.class);
        Optional<CompletableFuture<CommandFunction<ServerCommandSource>>> future = provider.map(function -> CompletableFuture.supplyAsync(
                        () -> function.create(identifier, commandDispatcher, source, list, executor),
                        executor
                ).thenCompose(f -> f)
        ...
        return future.orElseGet(() -> original.call(supplier, executor));
    }
}
